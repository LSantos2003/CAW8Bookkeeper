...
class GreenieBoard extends SlashCommand {
	...
	public override async run(
		...
		if (!name) {
			...
			userEntries = await app.users.collection.find({}).toArray();
		...
		let threeWireBoard: Record<string, number> = {};
		let userCount = 0;
		for (let entry of userEntries) {
			userCount++;
			if (entry.username == null) continue;

			let info = await app.calcOpAwards(entry.username);
			let wireCount = 0;

			info.opsAttended.forEach(op => {
				const member = op.members.find(m => m.name == entry.username);

				if (member.wire && member.wire == 3) {
					wireCount++;
				}
			});

			threeWireBoard[entry.username] = wireCount;

			if (userCount % 10 == 0 || userCount == userEntries.length) interaction.editReply(`${userCount} / ${userEntries.length}`);
		}

		let sortedThreeWireBoard = Object.fromEntries(Object.entries(threeWireBoard).sort(([, a], [, b]) => b - a));
		console.log(JSON.stringify(sortedThreeWireBoard));

		let topThreeWires = Object.fromEntries(Object.entries(sortedThreeWireBoard).slice(0, 20));
		...
		embed.addFields([
			...
			{ name: "Most Three Wires", value: JSON.stringify(topThreeWires) }
		]);
	...
	public override async handleAutocomplete({ interaction, app }: SlashCommandAutocompleteEvent<Application>) {
		...
		const timeSinceLastLookup = Date.now() - GreenieBoard.lastNameLookupTime;
		// 1 minute
		if (timeSinceLastLookup > 1000 * 60) {
			GreenieBoard.lastNameLookupTime = Date.now();

			const users = await app.users.get();
			GreenieBoard.lastNames = users.map(u => u.username);
		}

		const query = focusedValue.value.toLowerCase();
		const primaryOptions = GreenieBoard.lastNames.filter(n => n.toLowerCase().startsWith(query));
		const secondaryOptions = GreenieBoard.lastNames.filter(n => n.toLowerCase().includes(query) && !primaryOptions.includes(n));
		const options = [...primaryOptions, ...secondaryOptions].map(n => ({ name: n, value: n })).slice(0, 25);
		await interaction.respond(options);
	}
}

export default GreenieBoard;
